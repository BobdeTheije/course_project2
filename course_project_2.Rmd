...
### Top 10 Events causing fatalities or injuries

```{r}
par(mfrow = c(2,1))
ggplot(event_fatalities_top10) + geom_col(aes(EVENT, sum_fatalities, fill = EVENT))
ggplot(event_injuries_top10) + geom_col(aes(EVENT, sum_injuries, fill = EVENT))
```

It's obvious that Tornado's are most harmfull with respect to population health.
